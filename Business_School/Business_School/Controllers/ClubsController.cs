...
namespace Business_School.Controllers
{
    ...
    Autoriza por acción, no por controlador (a menos que todas las acciones compartan exactamente los mismos roles). La granularidad explícita es más limpia, segura y fácil de auditar.

     */
    [Authorize(Roles = RoleHelper.Admin + "," + RoleHelper.DepartmentManager + "," + RoleHelper.ClubLeader + "," + RoleHelper.Student)]
    public class ClubsController : Controller
    {
        ...
        [AllowAnonymous]//permitir anonimos
        public async Task<IActionResult> Details(int? id, string? returnUrl = null)
        {
            ...
            // Comprobar si el estudiante es miembro
            bool isMember = false;
            if (User.Identity?.IsAuthenticated == true && User.IsInRole("Student"))
            {
                ...
                {
                    isMember = club.StudentClubs.Any(sc => sc.UserStudentId == userId);
                }
            }
